    \noindent
    \section*{MES, DATEL - DATED}
    \markboth{MES, DATEL - DATED}{}
    \begin{tabularx}{\linewidth}{|c|X|X|X|}
        \hline
      & \textbf{\sffamily{L}} DATEL & \textbf{\sffamily{M}} DATEM & \textbf{\sffamily{X}} DATEX\\
      \hline 
      \hline 
      & SPECIALL  & SPECIALM      & SPECIALX   \\
      & FESTIVOL  & FESTIVOM      & FESTIVOX   \\
        &   &       &    \\
        &   &       &    \\
        &   &       &    \\
      \hline
      \hline 
      7 &   &       &    \\
        &   &       &    \\
      \hline
      8&   &       &    \\
        &   &       &    \\
      \hline
      9&   &       &    \\
        &   &       &    \\
      \hline
      10&   &       &    \\
        &   &       &    \\
      \hline
      11&   &       &    \\
        &   &       &    \\
      \hline
      12&   &       &    \\
        &   &       &    \\
      \hline
      13&   &       &    \\
        &   &       &    \\
      \hline
      14&   &       &    \\
        &   &       &    \\
      \hline
      15&   &       &    \\
        &   &       &    \\
      \hline
      16&   &       &    \\
        &   &       &    \\
      \hline
      17&   &       &    \\
        &   &       &    \\
      \hline
      18&   &       &    \\
        &   &       &    \\
      \hline
      19&   &       &    \\
        &   &       &    \\
      \hline
      20&   &       &    \\
        &   &       &    \\
      \hline
      21-24&   &       &    \\
        &   &       &    \\
      \hline  
      & MOONL\hspace{0.45em}PLANETL & MOONM\hspace{0.45em}PLANETM & MOONX\hspace{0.45em}PLANETX\\
      \hline   
    \end{tabularx}




    \newpage

        \noindent
        \section*{\phantom{Lorem}}
        \begin{tabularx}{\linewidth}{|c|X|X|p{2cm}|p{2cm}|}
            \hline
          & \textbf{\sffamily{J}} DATEJ & \textbf{\sffamily{V}} DATEV & \textbf{\sffamily{S}} \tiny{DATES} &   \textbf{\sffamily{D}} \tiny{DATED}\\
          \hline 
          \hline 
          & SPECIALJ  &   SPECIALV    & SPECIALS   & SPECIALD   \\
          & FESTIVOJ  &   FESTIVOV    & FESTIVOS   & FESTIVOD   \\
            &   &       &    &    \\
            &   &       &    &    \\
            &   &       &    &    \\
          \hline
          \hline 
          7&   &                  &    &    \\
            &   &       &    &    \\
          \hline
          8&   &                  &    &    \\
            &   &       &    &    \\
          \hline
          9&   &                  &    &   \\
            &   &       &     &   \\
          \hline
          10&   &                 &     &   \\
            &   &       &     &   \\
          \hline
          11&   &                 &    &   \\
            &   &       &    &   \\
          \hline
          12&   &                 &     &   \\
            &   &       &     &   \\
          \hline
          13&   &                 &    &   \\
            &   &       &    &   \\
          \hline
          14&   &                 &    &   \\
            &   &       &    &   \\
          \hline
          15&   &                 &    &   \\
            &   &       &    &   \\
          \hline
          16&   &                 &    &   \\
            &   &       &    &   \\
          \hline
          17&   &                 &    &   \\
            &   &       &    &   \\
          \hline
          18&   &                 &    &   \\
            &   &       &    &   \\
          \hline
          19&   &                 &     &   \\
            &   &       &    &   \\
          \hline
          20&   &              &    &   \\
            &   &       &    &   \\
          \hline 
          21-24&   &              &    &   \\
            &   &       &    &   \\
          \hline
                      & MOONJ\hspace{0.45em}PLANETJ & MOONV\hspace{0.45em}PLANETV & MOONS\hspace{0.45em}PLANETS & MOOND\hspace{0.45em}PLANETD\\
          \hline    
        \end{tabularx}
